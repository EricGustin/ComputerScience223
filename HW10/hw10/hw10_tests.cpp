...
TEST(BasicCollectionTest, RemoveElems) {
 ...
 RBTCollection<string,int> g;
 g.insert("d", 0);
 g.insert("e", 1);
 g.insert("i", 2); // right outside case w/ black or null sibling
 ASSERT_EQ(g.size(), 3);
 ASSERT_EQ(g.height(), 2);
 g.insert("h", 3); //right inside with red sibling
 ASSERT_EQ(g.height(), 3);
 g.insert("f", 4); // left outside w/ black or null sibling
 ASSERT_EQ(g.height(), 3);
 g.insert("ef", 5); // left outside with red sibling
 ASSERT_EQ(g.height(), 4);
 g.insert("efg", 6); // left inside w/ black or null sibling
 ASSERT_EQ(g.height(), 4);
 g.insert("eff", 7); // left inside with red sibling and right inside w/ black sibling
 ASSERT_EQ(g.height(), 4); //5?
 g.insert("eef", 8);
 g.insert("efff", 9); // right outside with red sibling and triggers a left inside w/ red sibling on the root
 ASSERT_EQ(g.height(), 5);
 ASSERT_EQ(g.size(), 10);

 RBTCollection<string,double> d;
 d.insert("f", 10.0);
 d.insert("v", 20.0);
 d.insert("z", 30.0);
 d.insert("j", 30.0);
...
TEST(BasicCollectionTest, CopyList) {
 ...
 w.insert("none", 2110);
 RBTCollection<string,int> w_copy = w;
 ...
 RBTCollection<string,int> empty;
 RBTCollection<string,int> empty_copy(empty);
 ASSERT_EQ(empty.size(), 0);
 ASSERT_EQ(empty_copy.size(), 0);
}
// Test 12
TEST(BasicCollectionTest, Assign) {
 RBTCollection<string,int> y;
 y.insert("gofish", 10);
 y.insert("safety", 20);
 y.insert("xyz", 30);
 y.insert("ergo", 40);
...
TEST(BasicCollectionTest, Negatives) {
 RBTCollection<double,string>* l = new RBTCollection<double,string>;
...
TEST(BasicCollectionTest, SizeZero) {
 RBTCollection<int,int>* m = new RBTCollection<int,int>;
 ASSERT_EQ(m->size(), 0);
