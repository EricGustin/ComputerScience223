...
using namespace std;
// insertion sort test
TEST(BasicsListTest, InsertionSort) {
 //random order
 LinkedListCollection<string,int> a;
 
 
 vector<string> sortedLanguages;
 a.quick_sort();
 a.keys(sortedLanguages);
 ASSERT_EQ(sortedLanguages.size(), 0);
 }
  a.insert("mine", 2018);
 ...
 b.insert("python", 1991);
 b.insert("perl", 1987);
 b.insert("pascal", 1970);
 b.insert("objectivec", 1983);
 b.insert("fortran", 1957);
 b.insert("cplusplus", 1983);
 b.insert("c", 1972);
 vector<string> languages2;
 b.insertion_sort();
 b.keys(languages2);
 for (int i = 0; i < languages2.size()-1; ++i) { 
  ASSERT_LE(languages2[i], languages2[i+1]);
 } 

 // already sorted
 LinkedListCollection<string,int> c;
  c.insert("c", 1972);
  c.insert("cplusplus", 1983);
  c.insert("fortran", 1957);
  c.insert("objectivec", 1983);
  c.insert("pascal", 1970);
  c.insert("perl", 1987);
  c.insert("python", 1991);
 vector<string> languages3;
 c.insertion_sort();
 c.keys(languages3);
 for (int i = 0; i < languages3.size()-1; ++i) { 
  ASSERT_LE(languages3[i], languages3[i+1]);
 } 
}
...
TEST(BasicsListTest, CorrectSize) {
 ...
 a->insertion_sort();
 a->keys(sortedStr);
 for (int i = 0; i < sortedStr.size()-1; ++i) { 
   ASSERT_LE(sortedStr[i], sortedStr[i+1]);
 }
  delete a;
